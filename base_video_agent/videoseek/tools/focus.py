...
def execute_focus(config: dict, parameters: dict) -> str:
    ...
    # Frame Sampling
    start_frame = min(max(int(start_time * vr.get_avg_fps()), 0), len(vr) - 1)
    end_frame = min(max(int(end_time * vr.get_avg_fps()), start_frame), len(vr) - 1)
    num_frames = min(end_frame - start_frame + 1, max_num_frames)
    ...
    for frame, timestamp in zip(frames, cur_timestamps):
        ...
        # img.show(title="Grid Frame Preview")
