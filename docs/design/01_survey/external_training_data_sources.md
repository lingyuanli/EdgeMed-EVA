...
2. **SLAKE English validation**: an independently versioned development set for
   choosing among frozen candidate methods. It is never mixed into training.
...
| Scale for low-cost SFT | high | modest | train on PMC-VQA; evaluate on SLAKE |
...
3. Build JSONL manifests with per-image SHA-256 and source metadata.
