...
- task：16,655 MCQ + 3,998 open-ended；七个复杂推理维度。
...
- expected command path：`python -m edgemed_bench.run --config configs/baseline/qwen35_4b_medcmr_b0.yaml`（待实现后冻结）。
...
- required download：官方 `dataset.zip`；固定 revision，顺序下载并校验 LFS SHA-256。
- hardware assumption：1× Tesla V100-SXM2-32GB，10 CPU cores，62GB RAM；正式开跑前重新核验可用磁盘。
...
- next item：下载前核验 archive 目录、解压规模、dataset schema 和 supplement evaluator prompt。
