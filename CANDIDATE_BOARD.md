...
|---:|---|---|---|---|---|---|---|---|---|
| 1 | `external-mcq-calibration-gate` | analysis/data | `a1-semantic-order-consensus` | explore | evaluation data / Tier1 | `next` | Prevents further policy tuning on exhausted PMC-512 while enabling a calibrated A2 Agent | n/a | admit a source-diverse, overlap-audited MCQ calibration/validation split before A2 design |
| 2 | `a1-semantic-order-consensus` | implementation | `m2a-semantic-option-sft` | explore | Agent/controller / Tier2 | `pilot_failed_archived` | Uses frozen M2a across all cyclic option views and votes by semantic option identity, targeting order sensitivity without further weight tuning | 58.0078%, equal to B0 and -4.8828 below M2a; invalid 4; order-invariance passed; 4 calls/question | archive; PMC-512 analysis-only; do not post-hoc select favorable views |
| 3 | `m2b-order-augmented-semantic-sft` | implementation | `m2a-semantic-option-sft` | debug | training data view / Tier2 | `pilot_failed_archived` | Keeps the successful option-text objective and decorrelates answer content from fixed letter positions using deterministic training-only rotations | consistency +8.5938, CI wholly positive; original -1.1719 and rotated -3.1250 accuracy points | archive; no seed search, SLAKE, or Med-CMR |
| 4 | `m2a-semantic-option-sft` | implementation | `qwen35-4b-medcmr-b0` | debug | supervised objective / Tier2 | `accuracy_pass_invariance_inconclusive` | Replaces answer-letter targets with option text to remove the confirmed label shortcut while preserving medical-domain adaptation | original +4.8828 and rotated +4.4922 points with CIs positive; consistency +1.7578 but CI [-3.1250, 6.6406] misses -1 non-inferiority margin | retain as best PMC accuracy parent; no Med-CMR test authorization |
| 5 | `transfer-failure-20260902` | analysis | `m1a-answer-qlora` | debug | evaluation/data view / Tier1 | `completed_h2_supported` | Distinguishes cross-source semantic retention from same-source/letter-token shortcuts without reopening Med-CMR | SLAKE exact +8.6420/F1 +5.5843; M1a rotation consistency -8.5938 vs B0 with CI wholly negative | stop letter-only SFT; hand off to M2a semantic option-content objective |
| 6 | `m1a-answer-qlora` | implementation | `qwen35-4b-medcmr-b0` | exploit | supervised objective / Tier2 | `test_failed_archived` | Tests whether admitted external medical MCQs improve domain answer alignment on one V100 | PMC dev 3/3 positive, but Med-CMR 24.3591% vs B0 27.1690%; -2.8100 points with CI entirely below zero; all seven dimensions regress, invalid becomes 0 | archive M1a; do not repeat test or tune from per-sample labels; redesign transfer gate before a new mechanism |
| 7 | `b1-evidence-answer-v2` | implementation | `b1-structured-json-v1` | debug | prompt + representation / Tier1 | `dev_failed_archived` | Tests the minimum evidence-attribution surface after v1 proved too complex for reliable zero-shot formatting | 512/512 strict JSON, but 39.8438% vs direct 57.6172%; paired delta -17.7734 points, CI entirely below zero | **archive for answer optimization; retain as negative result** |
