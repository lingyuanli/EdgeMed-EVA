...
| 2 | `m1a-answer-qlora` | implementation | `qwen35-4b-medcmr-b0` | explore | supervised objective / Tier2 | `code_ready_data_pending` | Tests whether 2k clean external medical MCQs can improve domain answer alignment on one V100 | assistant-only loss masking and language-only LoRA contract implemented; GPU backward not yet run | run bounded QLoRA smoke only after PMC admission |
